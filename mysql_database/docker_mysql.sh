...
sudo docker run --name SensorianHub_Database -v ~/SensorianHubDatabase:/var/lib/mysql -e MYSQL_ROOT_PASSWORD=sensorian_root_password -e MYSQL_DATABASE=SensorianHubSiteDB -e MYSQL_USER=Sensorian -e MYSQL_PASSWORD=sensorian_user_password -d mysql/mysql-server:latest
